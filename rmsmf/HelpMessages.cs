    ...
    public static class HelpMessages
    {
        ...
        private static string[] GetRmsmfHelpKorean()
        {
            return new string[]
            {
                ...
                "",
                "인코딩 자동 감지 모드:",
                "/j:0 일반 모드 (자체 처리로 감지할 수 없는 경우 타사 제품으로 감지)",
                "/j:1 자체 처리만으로 자동 감지",
                "/j:3 타사 제품(UTF.Unknown)만으로 자동 감지",
                "",
        ...
        private static string[] GetRmsmfHelpChineseSimplified()
        {
            return new string[]
            {
                ...
                "",
                "编码自动检测模式：",
                "/j:0 普通模式（如果无法通过自有处理检测，则使用第三方产品检测）",
                "/j:1 仅使用自有处理进行自动检测",
                "/j:3 仅使用第三方产品（UTF.Unknown）进行自动检测",
                "",
                "如果不指定，将使用普通模式。",
                "",
            };
        ...
        private static string[] GetRmsmfHelpChineseTraditional()
        {
            return new string[]
            {
                ...
                "",
                "編碼自動偵測模式：",
                "/j:0 普通模式（如果無法透過自有處理偵測，則使用第三方產品偵測）",
                "/j:1 僅使用自有處理進行自動偵測",
                "/j:3 僅使用第三方產品（UTF.Unknown）進行自動偵測",
                "",
                "如果不指定，將使用普通模式。",
                "",
            };
